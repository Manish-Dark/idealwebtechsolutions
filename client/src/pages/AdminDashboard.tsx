  ...
  return (
    <div style={{ padding: '40px' }}>
      <header style={{ marginBottom: '40px' }}>
        <h1 style={{ fontSize: '32px', fontWeight: 700, marginBottom: '8px' }}>Admin Overview</h1>
        <p style={{ color: 'var(--text-muted)' }}>Welcome back, {user?.name}. Here is what's happening today.</p>
      </header>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '24px', marginBottom: '40px' }}>
        <div className="glass-card">
          <p style={{ color: 'var(--text-muted)', marginBottom: '8px', fontSize: '14px' }}>Total Strength</p>
        ...
        <div className="glass-card">
          <p style={{ color: 'var(--text-muted)', marginBottom: '8px', fontSize: '14px' }}>Pending Approvals</p>
          <h2 style={{ fontSize: '28px', fontWeight: 700, color: stats.pendingLeaves > 0 ? 'var(--warning)' : 'inherit' }}>{stats.pendingLeaves}</h2>
        </div>
        <div className="glass-card">
          <p style={{ color: 'var(--text-muted)', marginBottom: '8px', fontSize: '14px' }}>Site Visits</p>
        ...
        <div className="glass-card">
          <p style={{ color: 'var(--text-muted)', marginBottom: '8px', fontSize: '14px' }}>Active Projects</p>
        ...
        {shortcuts.map(item => (
          <div 
            ...
            style={{ 
              ...
              border: item.badge ? '1px solid var(--warning)' : '1px solid var(--border)'
            }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  ...
                  {item.badge && <span style={{ backgroundColor: 'var(--warning)', color: 'white', fontSize: '10px', padding: '2px 8px', borderRadius: '10px', textTransform: 'uppercase' }}>{item.badge}</span>}
                </div>
                <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '4px' }}>{item.desc}</p>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              <span style={{ fontSize: '20px', fontWeight: 700 }}>{item.count}</span>
      ...
      <div className="glass-card" style={{ marginTop: '24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 32px' }}>
