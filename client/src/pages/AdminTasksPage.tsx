...
const AdminTasksPage: React.FC = () => {
  ...
  const fetchAllData = useCallback(async () => {
    try {
      ...
      const [tasksRes, usersRes, customersRes] = await Promise.all([
        axios.get(`http://localhost:5000/api/admin/tasks?t=${Date.now()}`, config),
        axios.get(`http://localhost:5000/api/admin/users?t=${Date.now()}`, config),
        axios.get(`http://localhost:5000/api/admin/customers?t=${Date.now()}`, config)
      ]);
  ...
  const handleAssignTask = async (e: React.FormEvent) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user?.token}` } };
      await axios.post('http://localhost:5000/api/admin/tasks', taskForm, config);
  ...
  const handleDeleteTask = async (id: string) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user?.token}` } };
      await axios.delete(`http://localhost:5000/api/admin/tasks/${id}`, config);
      fetchAllData();
    } catch (err) {
  ...
  return (
    <div style={{ padding: '40px' }}>
      <header style={{ marginBottom: '40px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ fontSize: '32px', fontWeight: 700, marginBottom: '8px' }}>Task Management</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '32px' }}>
        ...
        <div className="glass-card" style={{ padding: '0', height: 'fit-content' }}>
          <div style={{ padding: '24px', borderBottom: '1px solid var(--border)' }}>
            <h3 style={{ fontSize: '18px', fontWeight: 700 }}>Deployment Pipeline</h3>
          </div>
